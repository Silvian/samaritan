$(document).ready(function(){
    ...
    if(rolesData) {
        ...
        var guests_table = $('#guests-list').DataTable({
        'ajax': {
            "type"   : "GET",
            "url"    : '/api/guests/getActive',
    ...
    $("#add-guest-button").click(function(event) {
        clearFields();
        $('#member-id').val("");
        $('#address-id').val("");
        $('#create-address').show();
    ...
    $("#save-member").click(function(event) {
        addMember(guests_table);
    });

    $('#guests-list').on("click", 'button', function() {
        clearFields();
